...
func (c *ActionsOIDCClient) GetJWT() (*ActionsJWT, error) {
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("received non-200 from jwt api: %s\n%s", http.StatusText((response.StatusCode)), string(rawBody))
	}
...
func (j *ActionsJWT) Parse() {
	fmt.Println("parsing jwt: ", j.Value)

	var err error
	j.ParsedToken, err = jwt.Parse(j.Value, func(token *jwt.Token) (interface{}, error) {
		fmt.Println("JWT method alg: ", token.Method.Alg())
		switch method := token.Method.(type) {
		case *jwt.SigningMethodHMAC:
			fmt.Println("JWT method: HMAC", method.Name)
		case *jwt.SigningMethodRSA:
			fmt.Println("JWT method: RSA", method.Name)
		default:
